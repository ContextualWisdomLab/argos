...
- Windows CLI 인증 URL을 `cmd.exe /c start`에 전달하던 경로를 제거했습니다. 인증 URL은 `http:`/`https:`만 허용하고 정규화한 뒤 `rundll32.exe url.dll,FileProtocolHandler`에 별도 인자로 전달하며, `%VAR%`·따옴표·셸 메타문자가 명령 인터프리터에 도달하지 않는 회귀 테스트를 추가했습니다. 거부된 원격 URL은 터미널에 그대로 출력하지 않고 사용자가 재로그인 또는 서버 설정 확인이라는 다음 조치를 알 수 있도록 오류 문구도 바꿨습니다.
