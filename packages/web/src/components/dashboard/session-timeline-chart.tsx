...
function getToolSummaryForRange(
  ...
  // 첫 번째 bar는 prevTimestamp가 0이므로 해당 bar 이전의 모든 이벤트를 포함
...
export function SessionTimelineChart({
  ...
  // ⚡ Bolt: usageTimeline 배열을 순회하며 차트 데이터를 생성할 때
  // 매 반복마다 발생하는 new Date() 문자열 파싱 오버헤드를 최소화하기 위해
  // 루프 바깥에서 시작 시간을 미리 파싱하고, 이전 타임스탬프를 기억하여 객체 생성을 줄임.
  const chartData: ChartDataItem[] = useMemo(() => {
    const sessionStartMs = new Date(sessionStartedAt).getTime();
    return usageTimeline.map((u, index) => {
      const currentTimestamp = new Date(u.timestamp).getTime();
      const prevTimestamp =
        index > 0 ? new Date(usageTimeline[index - 1]!.timestamp).getTime() : 0;
      const item: ChartDataItem = {
        relativeTime: formatRelativeTime(currentTimestamp, sessionStartMs),
        input: u.inputTokens,
        output: u.outputTokens,
        cost: u.estimatedCostUsd,
        model: u.model,
        toolSummary: getToolSummaryForRange(
          currentTimestamp,
          prevTimestamp,
          toolCalls,
        ),
