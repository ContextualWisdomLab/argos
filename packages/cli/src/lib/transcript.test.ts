...
function writeJsonl(dir: string, lines: object[]): string {
  const path = // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    join(dir, "transcript.jsonl");
  writeFileSync(
    path,
    lines
      .map((l) => JSON.stringify(l)) // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
      .join("\n"),
    "utf8",
  );
  return path;
...
describe("readTranscriptLines", () => {
  ...
  it("빈 줄은 제거한다", async () => {
    const path = // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
      join(tempDir, "empty-lines.jsonl");
    writeFileSync(path, '{"type":"human"}\n\n{"type":"assistant"}\n', "utf8");
