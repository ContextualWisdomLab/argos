...
describe('LoginRequestSchema', () => {
  ...
  it('비밀번호가 1025자면 실패한다 (max 1024 경계)', () => {
    expect(
      LoginRequestSchema.safeParse({ email: 'a@b.com', password: 'x'.repeat(1025) }).success,
    ).toBe(false)
  })
})
...
describe('RegisterRequestSchema', () => {
  ...
  it('비밀번호가 1025자면 실패한다 (max 1024 경계)', () => {
    expect(
      RegisterRequestSchema.safeParse({ email: 'a@b.com', password: 'x'.repeat(1025), name: 'k' }).success,
    ).toBe(false)
  })
})
