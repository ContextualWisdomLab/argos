...
## 2025-01-20 - 닫기 버튼 내부 아이콘 화면 읽기 중복 방지
**Learning:** `aria-label`이 설정된 닫기 버튼 내부에 시각적 의미만 전달하는 아이콘(예: X)이 있을 때, 해당 아이콘에 `aria-hidden="true"`를 설정하지 않으면 일부 스크린 리더 환경에서 아이콘을 불필요하게 읽어 접근성 경험을 저해할 수 있습니다.
**Action:** 접근성 레이블(`aria-label`)이 제공된 대화형 요소 내에 있는 장식성 아이콘들에는 항상 `aria-hidden="true"` 속성을 추가하여 스크린 리더 사용자에게 간결하고 명확한 정보만 제공하도록 합니다.
