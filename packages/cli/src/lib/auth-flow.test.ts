...
describe('auth-flow', () => {
  ...
  it('opens browser using start on win32 safely with spawn', async () => {
    ...
    mockApiRequest.mockResolvedValueOnce({ state: 'state123', authUrl: 'http://example.com/&calc|echo;test<hello>world^yes' }) // Step 1
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'cmd.exe',
      ['/c', 'start', '""', 'http://example.com/^&calc^|echo^;test^<hello^>world^^yes'],
