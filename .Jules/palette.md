...
## 2024-05-24 - 인터랙티브 요소 내 장식용 아이콘 스크린 리더 노이즈 제거
**Learning:** 스크린 리더가 버튼이나 링크 내부에 있는 장식용 아이콘을 불필요하게 읽어 사용자 경험을 저하시키는 패턴이 발견됨.
**Action:** 접근성 향상을 위해 기능이 없고 시각적 의미만 있는 아이콘에는 항상 `aria-hidden="true"` 속성을 추가하여 스크린 리더 노이즈를 줄인다.
