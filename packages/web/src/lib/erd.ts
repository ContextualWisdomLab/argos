...
function assertNoStatementTerminator(typeStr: string): void {
  if (typeStr.includes(";")) {
    throw new Error(
      `Invalid column type '${typeStr}': Statement terminators like ';' are not allowed to prevent SQL injection.`,
    );
  }
}
