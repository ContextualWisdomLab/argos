...
describe('AdminDashboard accessibility feedback', () => {
  ...
  it('exposes selection, focus styling, and stable copy-action feedback', async () => {
    render(<AdminDashboard />)
  ...
  it('keeps copy retryable and provides the next action when clipboard access fails', async () => {
    ...
    expect(screen.queryByTestId('check-icon')).not.toBeInTheDocument()
    expect(
      screen.getByText('Unable to copy reset link. Select the generated link and copy it manually.')
    ).toBeInTheDocument()
    expect(screen.getByRole('status')).toBeEmptyDOMElement()
  })
})
