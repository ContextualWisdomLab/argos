  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // Impact: Completely avoids array allocations, eliminating GC overhead when aggregating large daily rollups.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] for...in iterations completely avoid array allocations, eliminating heap thrashing
    for (const k in r.skillCounts) {
