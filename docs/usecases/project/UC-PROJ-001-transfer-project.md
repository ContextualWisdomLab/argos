...
3. (User · UI) "계속" 을 클릭한다.
4. (System · UI) 이동 버튼을 `aria-busy` 로 잠그고 접근 가능한 이름을 `이동 중…` 으로 바꾼다. 구매자는 완료될 때까지 다시 누르지 않는다.
5. (System · API) `POST /api/projects/{projectId}/transfer { targetOrgSlug }` 를 호출한다.
6. (System) 호출자가 출발 org 와 대상 org 양쪽에서 role === 'OWNER' 임을 검증한다.
7. (System · DB) 단일 `db.$transaction` 안에서 `Project.orgId` 를 targetOrg.id 로 갱신하고 `ProjectMember.deleteMany({ where: { projectId } })` 를 실행한다.
8. (System · API) 200 + `{ project: { id, orgId, orgSlug, name, slug, createdAt } }` 를 반환한다.
9. (System · UI) `/dashboard/{targetOrgSlug}/settings/projects` 로 라우트를 바꾸고 "Transferred" 토스트를 표시한다.

## 확장 (Extensions)

- 6a. 호출자가 출발 또는 대상 org 에서 OWNER 가 아님 →
  - 6a.1. (System · API) 403 + `{ error: { code: 'FORBIDDEN', message } }` 반환. M1 으로 어떤 row 도 변경되지 않음.
  - 6a.2. (System · UI) 에러 토스트 표시 후 종료.
- 6b. project 또는 targetOrg 가 존재하지 않음 →
  - 6b.1. (System · API) 404 + `{ error: { code: 'NOT_FOUND', message } }` 반환. M1.
  - 6b.2. (System · UI) 에러 토스트 표시 후 종료.
- 6c. `project.orgId === targetOrg.id` (same_org) →
  - 6c.1. (System · API) 200 + 현재 project 상태를 그대로 반환. 어떤 변경도 발생하지 않음.
  - 6c.2. (System · UI) UI 에서는 대상 org 후보 dropdown 이 현재 org 를 제외하므로 일반 사용자는 이 경로에 도달하지 않는다. API 직접 호출 시에만 발생.
- 7a. 트랜잭션 중 `(orgId, slug)` unique 위반 (대상 org 에 동일 slug 프로젝트 존재) →
  - 7a.1. (System · API) 409 + `{ error: { code: 'PROJECT_SLUG_CONFLICT', message: '대상 org 에 같은 이름(slug)의 프로젝트가 이미 있습니다. 한쪽 이름을 먼저 변경한 뒤 다시 시도하세요.' } }` 반환. M2 로 부분 commit 없음.
  - 7a.2. (System · UI) 모달 안에 Alert 로 안내 후 종료.

## 기술/데이터 변형

- V1. T2 (API 직접 호출) 진입 시 1~4 단계를 건너뛰고 5 단계로 진입한다. 6c 경로의 same_org noop 도 이 변형에서만 의미가 있다.
