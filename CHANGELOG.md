...
- 세션 CSV 내보내기에서 사용자·프로젝트·프롬프트 문자열이 `=`, `+`, `-`, `@`, 탭, CR, LF로 시작할 경우 스프레드시트 수식으로 실행되지 않도록 중립화하고 RFC 4180 인용·따옴표 이스케이프를 공통 serializer로 고정했습니다. 실제 내보내기 route가 이 serializer를 사용하며 위험한 모든 선행 marker를 회귀 테스트로 검증합니다.
