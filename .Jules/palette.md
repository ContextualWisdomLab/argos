...
## 2024-11-21 - 이벤트 디테일 닫기 버튼 스크린 리더 접근성 개선
**Learning:** EventDetail 컴포넌트의 닫기 버튼처럼 아이콘만 있는 버튼에 aria-label이 있더라도, 내부 아이콘 컴포넌트(예: <X>)에 aria-hidden="true"를 지정하지 않으면 스크린 리더가 불필요하게 아이콘의 존재를 읽어 사용자에게 혼란을 줄 수 있습니다.
**Action:** 아이콘 전용 버튼을 만들 때는 시각적 요소인 아이콘에 항상 aria-hidden="true"를 추가하여 스크린 리더가 오직 버튼의 의미(aria-label)만 명확하게 전달하도록 해야 합니다.
