...
const FIXTURE = [
  ...
  },
  {
    ...
    type: "turn_context",
    payload: { turn_id: "t1", cwd: "/x", model: "gpt-5.5" },
  },
  {
    timestamp: "2026-05-26T00:00:02.000Z",
    type: "event_msg",
    payload: { type: "user_message", message: "적절히 commit push한거 맞아?" },
  },
  {
    timestamp: "2026-05-26T00:00:03.000Z",
    type: "event_msg",
    payload: { type: "agent_message", message: "Checking git status first." },
  },
  {
  ...
  {
    timestamp: "2026-05-26T00:00:05.500Z",
    type: "response_item",
    payload: {
  ...
  {
    timestamp: "2026-05-26T00:00:06.200Z",
    type: "response_item",
    payload: {
  ...
  {
    timestamp: "2026-05-26T00:00:03.500Z",
    type: "event_msg",
    payload: {
      type: "token_count",
      info: {
  ...
  {
    timestamp: "2026-05-26T00:00:07.000Z",
    type: "event_msg",
    payload: {
      type: "token_count",
      info: {
...
describe("transcript-codex", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(
      path,
      FIXTURE.map((l) => JSON.stringify(l)).join("\n"),
      "utf8",
    );
  });
  ...
  it("extractMessages: HUMAN/ASSISTANT/TOOL 매핑 + 결과 backfill", async () => {
    ...
    const byRole = msgs.reduce<Record<string, number>>((a, m) => {
  ...
  it("존재하지 않는 파일 → null / []", async () => {
    expect(
      await extractUsageFromCodexTranscript("/no/such/file.jsonl"),
    ).toBeNull();
    expect(
      await extractMessagesFromCodexTranscript("/no/such/file.jsonl"),
    ).toEqual([]);
  });
});
