...
## 2024-12-05 - 텍스트 버튼 및 장식용 아이콘 접근성 개선
**Learning:** 디자인 시스템의 기본 `<Button>`을 사용하지 않고 직접 만든 텍스트 `<button>`(예: 'Log out' 버튼)에 `hover` 스타일만 지정하고 키보드 포커스 스타일을 누락하는 경우가 있습니다. 이 경우 키보드 사용자는 포커스 위치를 잃게 됩니다. 또한 버튼 텍스트 옆에 추가된 장식용 아이콘(`<PlusIcon>`)이 크기 제어 없이 렌더링되거나 `aria-hidden`이 누락되어 스크린 리더에서 불필요하게 읽히는 문제가 있었습니다.
**Action:** 텍스트 링크 형태의 `<button>`을 직접 렌더링할 때는 항상 `focus-visible:outline-none focus-visible:ring-2` 등 명시적인 포커스 스타일을 추가해야 합니다. 또한 버튼 내 장식용 아이콘에는 반드시 `aria-hidden="true"` 속성과 명시적인 크기 클래스(예: `size-4 shrink-0`)를 지정하여 시각적 균형과 스크린 리더 접근성을 함께 챙겨야 합니다.
