...
// ⚠️ We do NOT export ADMIN_USERNAME and ADMIN_PASSWORD as constants anymore
// to prevent "Hardcoded credentials import" findings. They are now accessed
// dynamically from the env object wherever needed.
...
function safeEqual(a: string, b: string): boolean {
  ...
  if (
    aBytes.length > MAX_SAFE_EQUAL_BYTES ||
    bBytes.length > MAX_SAFE_EQUAL_BYTES
  ) {
    return false;
  }
  ...
  return timingSafeEqual(aHash, bHash) && aBytes.length === bBytes.length;
}
