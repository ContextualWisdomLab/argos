  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // [Bolt: Performance Optimization] Use for...in guarded by Object.hasOwn() instead of Object.keys() in hot paths.
        // Impact: Completely avoids array allocation for keys, eliminating GC overhead when aggregating large daily rollups.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] for...in loops guarded by Object.hasOwn() avoid array allocation completely, eliminating GC overhead in hot paths
    for (const k in r.skillCounts) {
