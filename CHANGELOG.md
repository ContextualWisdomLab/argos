...
- 날짜 범위 preset 버튼에 구체적인 접근 가능한 이름을 제공하고, 기본 7일 및 클릭으로 만든 7/30/90/전체 기간을 포함 일수 기준으로 계산하여 URL과 `aria-pressed` 선택 상태가 일치하도록 회귀 계약으로 고정했습니다. 잘못된 `from`/`to` URL 값은 render를 중단하지 않고 기본 7일 범위로 fail closed 합니다.
