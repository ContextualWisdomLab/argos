...
describe("ERDModel", () => {
  ...
  describe("Column Management", () => {
    ...
    it("should reject invalid characters in column types to prevent DDL injection", () => {
      model.addTable("users");
      expect(() =>
        ...
        "Column type 'TEXT\n);\nDROP TABLE users;--' contains invalid characters.",
      );
      expect(() =>
        model.addColumn("users", {
          name: "age",
          type: "INT; SELECT * FROM users",
        }),
      ).toThrowError(
        "Column type 'INT; SELECT * FROM users' contains invalid characters.",
      );
    });
  });
