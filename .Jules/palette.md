...
## 2024-09-24 - Add icons to actions
**Learning:** Adding icons to plain-text actions like "Log Out" helps visually anchor the interactive elements. When modifying standard layout buttons to include an icon, you must convert the plain-text container to a flex container (`flex items-center gap-2`) or use proper margin utility classes (`mr-2`) to keep the icon aligned nicely with the text. `aria-hidden="true"` is also required for purely decorative action icons when visible text provides the accessible name.
**Action:** Always check the container's display and layout properties when injecting an icon, and include `aria-hidden="true"` to prevent redundant screen reader announcements.
