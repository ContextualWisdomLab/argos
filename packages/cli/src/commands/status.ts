...
export const makeStatusCommand: CommandFactory =
  (deps) => async () => {
    ...
    // resolve against cwd to ensure clean path normalization
    const safeCwd = deps.cwd()
    const claudePath = join(safeCwd, '.claude', 'settings.json')
    const codexPath = join(safeCwd, '.codex', 'hooks.json')
