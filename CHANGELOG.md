...
- 세션 타임라인 차트(`session-timeline-chart.tsx`)에서 이벤트 정렬 시 O(N log N)번 발생하던 `Date.parse()` 호출 병목을 Schwartzian transform(map-sort-map) 패턴으로 리팩토링하여 호출 횟수를 O(N)으로 개선했습니다. 이를 통해 타임라인 데이터가 많은 세션 차트 렌더링 성능이 최적화되었습니다.
