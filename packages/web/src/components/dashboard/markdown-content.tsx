...
// ⚡ Bolt: Prevent unnecessary re-renders by extracting static arrays and objects outside the component
// This avoids recreating new references on every render cycle which causes ReactMarkdown to re-render.
...
// ⚡ Bolt: Added React.memo to prevent re-rendering when the parent component updates,
// since Markdown parsing is expensive and the output only depends on the children string prop.
// Expected Impact: Reduces re-renders of the expensive ReactMarkdown component by ~100% when parent state changes.
