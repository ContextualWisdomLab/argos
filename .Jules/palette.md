...
## 2024-05-24 - 아이콘 전용 버튼을 디자인 시스템 Button으로 교체하고 툴팁 추가
**Learning:** 아이콘만 있는 기본 `<button>` 태그는 툴팁이 시각적으로 누락되고 포커스 상태(focus-visible)의 디자인이 일관되지 않는 경우가 많습니다.
**Action:** 항상 공유된 `<Button>` 컴포넌트를 사용하여 일관성을 유지하고, 스크린 리더를 위한 `aria-label`과 마우스 호버 시 시각적 힌트를 위한 `title` 속성을 모두 제공해야 합니다.
