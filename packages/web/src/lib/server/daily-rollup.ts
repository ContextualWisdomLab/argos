  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // Impact: Completely avoids array allocations, reducing GC overhead during large aggregations.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] for...in avoids intermediate array allocations from Object.keys(), reducing heap thrashing in hot loops
