...
## 2025-08-25 - [Accessibility] 버튼 포커스 시 접근성 피드백 개선
**Learning:** 포커스된 버튼의 텍스트(aria-label/text)를 동적으로 변경하여 상태 피드백('복사됨')을 제공하는 것은 스크린 리더 환경에서 불안정하게 읽힐 수 있습니다.
**Action:** 버튼의 시각적 라벨과 접근 가능한 이름(accessible name)을 고정하고, 상태 변화를 알리기 위해 `role="status"` 와 `aria-atomic="true"` 속성을 가진 시각적으로 숨겨진 요소를 형제로 배치하여 해결합니다.
