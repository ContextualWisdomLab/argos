...
## 2024-08-20 - 상호작용 요소에 대한 접근성 높은 성공 피드백
**Learning:** 현재 포커스된 버튼의 접근성 이름(aria-label)을 동적으로 변경하는 것은 스크린 리더에서 불안정하게 작동할 수 있습니다. 상호작용 요소의 시각적/접근성 라벨을 정적으로 유지하고, 별도의 형제 요소로 `role="status"` 및 `aria-atomic="true"` 속성을 가진 컨테이너를 사용하면 상태 변화를 스크린 리더에 더 안정적으로 전달할 수 있습니다.
**Action:** 성공 상태를 보여주는 요소를 만들 때, 상호작용 요소의 aria label을 고정하고 화면에 보이지 않는 `role="status"` 컨테이너를 사용하여 스크린 리더에 상태 변화를 알리십시오.
