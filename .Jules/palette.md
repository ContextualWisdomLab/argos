...
## 2024-11-21 - 비동기 작업 버튼의 로딩 상태 시각적/접근성 개선
**Learning:** 여러 페이지(예: 로그인 폼, 비밀번호 재설정 폼, 프로젝트/조직 생성 및 삭제 모달)에서 비동기 작업을 수행하는 버튼들이 `disabled={loading}` 상태만 가지고 있고 시각적으로 로딩 중임을 명확히 보여주는 인디케이터(예: 스피너)가 없었습니다. 텍스트만 "생성 중..." 등으로 변경되는 것보다 애니메이션되는 아이콘이 있으면 사용자가 작업 진행 상황을 훨씬 더 직관적으로 인지할 수 있습니다.
**Action:** `lucide-react`의 `Loader2` 아이콘을 `animate-spin` 클래스와 함께 사용하여 텍스트 옆에 추가하는 패턴을 일괄 적용했습니다. 이때 스크린 리더가 불필요하게 아이콘을 읽지 않도록 `aria-hidden="true"`를 추가하는 것이 필수적입니다.
