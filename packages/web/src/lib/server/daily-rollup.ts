  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // Impact: Completely avoids array allocation for keys, reducing GC overhead and memory thrashing when aggregating large daily rollups.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] for...in iterations completely avoid array allocations for keys, further reducing heap thrashing
    for (const k in r.skillCounts) {
      if (Object.hasOwn(r.skillCounts, k)) {
