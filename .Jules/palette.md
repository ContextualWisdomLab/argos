...
## 2024-11-20 - 비동기 폼 버튼 로딩 스피너 추가 및 시각적 피드백 개선
**Learning:** 로그인, 회원가입, 비밀번호 재설정과 같은 비동기 폼 제출 시 버튼이 비활성화되는 것만으로는 사용자에게 충분한 피드백을 제공하지 못할 수 있습니다. 텍스트 변경(예: 'Sign in' -> 'Signing in...')과 더불어 로딩 스피너 아이콘(예: Loader2)을 제공하면, 현재 작업이 진행 중임을 훨씬 명확하게 전달하여 사용자의 불안감을 줄이고 중복 클릭을 예방하는 데 효과적입니다. 이때 로딩 아이콘은 순수 장식용이므로 `aria-hidden="true"`를 부여하여 스크린 리더 사용자의 혼란을 방지해야 합니다.
**Action:** 비동기 동작이 수반되는 주요 버튼에는 항상 명시적인 시각적 로딩 상태(스피너 아이콘 등)를 추가하고, 스크린 리더가 불필요하게 읽지 않도록 접근성 속성(`aria-hidden="true"`)을 챙깁니다.
