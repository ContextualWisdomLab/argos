...
export async function getWeeklyReport(
  ...
  // Insights — delegation
  // ⚡ Bolt Optimization: Single-pass iteration replacing multiple .reduce passes and
  // intermediate Object.values allocations for performance.
  ...
  for (const r of thisWeekRollups) {
    ...
    for (let i = 0; i < skillKeys.length; i++) {
      totalSkillCalls += r.skillCounts[skillKeys[i]]
      distinctSkillsThisWeek.add(skillKeys[i])
    }
  }
