...
## 2024-08-02 - Decorative Icons in Interactive Elements
**Learning:** When using icons (e.g., from `lucide-react`) inside interactive elements like `<button>` that also contain visible text, screen readers may announce the icon redundantly or incorrectly if not explicitly hidden. It's crucial to consistently apply `aria-hidden="true"` to these purely decorative elements to improve the experience for screen reader users without affecting the visual layout.
**Action:** Always verify that `<svg>` or icon component tags inside interactive elements include `aria-hidden="true"` if they are accompanied by text or if the parent element already has a descriptive `aria-label`.
