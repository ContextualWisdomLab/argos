...
export async function getWeeklyReport(
  ...
  // ⚡ Bolt: Single pass iteration over rollups avoids multiple reduce overhead
  for (const r of thisWeekRollups) {
    for (const count of Object.values(r.agentCounts)) {
      totalAgentCalls += count
    }
    for (const [skillName, count] of Object.entries(r.skillCounts)) {
      totalSkillCalls += count
      distinctSkillsThisWeek.add(skillName)
    }
  }
