...
## 2024-10-27 - Decorate Icons in Interactive Elements
**Learning:** Adding `aria-hidden="true"` to purely decorative icons (like `lucide-react`'s Chevron icons) within interactive elements (buttons, links) that already have semantic state or labels prevents screen readers from redundantly announcing the icon's generic name. This is a crucial, easily overlooked accessibility pattern.
**Action:** Always verify if an icon conveys unique information. If the surrounding element provides context (e.g., via `aria-label` or `aria-expanded`), ensure the icon has `aria-hidden="true"`.
