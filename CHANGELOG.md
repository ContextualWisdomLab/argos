...
- 날짜 범위 프리셋을 `Date range presets` 그룹으로 묶고, `7d`·`30d`·`90d`·`ALL`의 보이는 이름을 유지한 채 별도 설명을 연결했습니다. 각 프리셋은 광고한 7·30·90·3,650개의 포함 달력 날짜와 일치하며, URL의 날짜가 누락·비정상·실재하지 않거나 역순이면 UI를 깨뜨리는 대신 안전한 기본 7일 범위로 복구합니다. 세션 목록의 아이콘 전용 삭제 액션은 제목이 있는 경우 `세션 삭제: <제목>`으로 대상을 식별하고, 제목이 없으면 기존의 간결한 이름으로 안전하게 폴백합니다.
