...
## 2024-06-25 - 세그먼트 컨트롤에 `aria-pressed` 및 키보드 접근성 개선
**Learning:** DateRangePicker의 프리셋 버튼과 같이 커스텀 버튼으로 구성된 세그먼트 컨트롤은 네이티브 라디오 버튼과 달리 내장된 상태 의미를 가지지 않습니다. 시각적 단서(예: 활성 프리셋의 배경색 변경)에만 의존하면 스크린 리더 사용자에게 상태 변화를 제대로 전달할 수 없습니다.
**Action:** 토글이나 세그먼트 제어 버튼에는 항상 `aria-pressed={isActive}` 속성을 추가하여 상태를 명확히 하고, 키보드 사용자도 쉽게 탐색할 수 있도록 명시적인 포커스 스타일(`focus-visible:ring-2` 등)을 적용해야 합니다.
