...
vi.mock('@/components/ui/card', () => ({
  ...
  CardTitle: ({ children }: React.HTMLAttributes<HTMLDivElement>) => <div>{children}</div>,
  CardDescription: ({ children }: React.HTMLAttributes<HTMLDivElement>) => <div>{children}</div>,
...
vi.mock('@/components/ui/alert', () => ({
  Alert: ({ children }: React.HTMLAttributes<HTMLDivElement>) => <div>{children}</div>,
  AlertTitle: ({ children }: React.HTMLAttributes<HTMLDivElement>) => <div>{children}</div>,
  AlertDescription: ({ children }: React.HTMLAttributes<HTMLDivElement>) => <div>{children}</div>
}))
...
describe('AdminDashboard', () => {
        ...
        return Promise.resolve({
          ...
          json: () => Promise.resolve({
            users: [{
              ...
              memberships: [{ role: 'OWNER', organization: { name: 'Org', slug: 'org' } }]
            }]
          })
        })
      }
  ...
  it('disables impersonation for ADMIN roles (OWNER or MANAGER)', async () => {
    ...
    const impersonateBtn = await screen.findByRole('button', { name: /Cannot impersonate admins/i })
  ...
  it('allows copying password reset link', async () => {
    ...
    const createLinkBtn = screen.getByRole('button', { name: /Create reset link/i })
    ...
    await waitFor(() => {
      expect(screen.getByDisplayValue('http://localhost/reset/fake')).toBeInTheDocument()
    })
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith('http://localhost/reset/fake')
