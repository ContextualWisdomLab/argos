...
## 2024-11-20 - Decorative Icons in Interactive Elements
**Learning:** Decorative icons (like chevrons in buttons or selects) without `aria-hidden="true"` can cause screen readers to announce unnecessary information, polluting the accessible name of the interactive element.
**Action:** Always add `aria-hidden="true"` to decorative icons, especially those within interactive elements like buttons, links, and selects, where the context is already provided by text or `aria-label`.
