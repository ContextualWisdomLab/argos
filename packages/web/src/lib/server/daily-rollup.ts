  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // [Bolt: Performance Optimization] Use for...in loop instead of Object.keys() in hot paths.
        // Impact: Completely avoids array allocation for object keys, significantly reducing GC overhead when aggregating large daily rollups.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] for...in loops avoid internal array allocations completely, reducing heap thrashing
