...
## 2024-08-07 - 비동기 작업 버튼 피드백 강화
**Learning:** 모달 내에서 조직 생성/삭제와 같은 비동기 작업 시, 텍스트 변경('생성 중...')만으로는 사용자가 로딩 상태를 명확히 인지하기 어려울 수 있으며, 시각적 피드백(스피너)을 추가하면 UX가 크게 개선됨을 확인했습니다.
**Action:** 앞으로 비동기 제출 버튼에는 항상 로딩 스피너 아이콘(예: Loader2)을 함께 사용하여 명확한 상태 피드백을 제공합니다.
