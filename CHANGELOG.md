...
- CLI 로그인 권한 확인 화면의 기본 HTML 버튼을 디자인 시스템의 공용 `Button` 컴포넌트로 교체하고, 허용·거부 요청을 각각 진행 상태로 표시합니다. 성공·거부 상태는 스크린 리더에 알리고, 실패 시에는 터미널로 돌아가 CLI 로그인을 다시 시작하라는 다음 행동을 명확히 안내합니다.
