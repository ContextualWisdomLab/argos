...
## 2025-01-20 - 주간 네비게이터 아이콘에 aria-hidden 추가
**Learning:** `WeekNavigator` 컴포넌트의 이전/다음 화살표와 같이 시각적 장식으로만 사용되는 아이콘(`ChevronLeft`, `ChevronRight`)에 `aria-hidden="true"`가 누락되면, 스크린 리더가 불필요하게 아이콘을 읽어들여 사용자 경험을 저해할 수 있습니다.
**Action:** 순수하게 시각적 장식 목적으로 사용되는 `lucide-react` 아이콘이나 SVG 요소에는 반드시 `aria-hidden="true"` 속성을 추가하여 접근성 트리를 깨끗하게 유지해야 합니다.
