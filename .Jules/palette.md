...
## 2024-09-21 - Add Loading Spinners to Auth Forms
**Learning:** Auth forms in this app were lacking visual feedback during async submission (only changing text, not showing a spinner), which could lead to confusion about whether an action was processing. Adding a standard lucide-react Loader2 spinner with `aria-hidden="true"` improves visual feedback without polluting the accessible name for screen readers.
**Action:** Next time, always check form submit buttons for loading states and add both visual (spinner) and semantic (aria-hidden on decorative icons) enhancements to improve the UX during async actions.
