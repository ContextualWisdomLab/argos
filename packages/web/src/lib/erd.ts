...
const VALID_SQL_TYPE = /^[a-zA-Z0-9_(),\s]+$/;
...
function assertValidColumnType(type: string): void {
  if (!VALID_SQL_TYPE.test(type)) {
    throw new Error(`Column type '${type}' contains invalid characters.`);
  }
}
...
export class ERDModel {
  ...
  addColumn(tableName: string, column: Column): void {
    ...
    assertValidColumnType(column.type);
