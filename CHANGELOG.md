...
- 세션 CSV 내보내기에서 사용자 제공 텍스트가 `=`, `+`, `-`, `@`, 탭·개행 또는 전각 수식 기호로 시작할 때 스프레드시트 수식으로 해석되지 않도록 텍스트 필드 경계를 보강했습니다. 숫자 필드는 기존 숫자 표현을 유지하고, 쉼표·따옴표·개행에 대한 CSV 인용 규칙과 BOM(U+FEFF) 회귀 검증을 함께 유지합니다.
