...
describe("POST /api/events — WU-4 응답 shape", () => {
  beforeEach(() => {
    ...
    vi.mocked(db.claudeSession.upsert).mockResolvedValue(
  ...
  it("잘못된 payload는 중첩된 VALIDATION_ERROR 계약을 반환한다", async () => {
  ...
  it("존재하지 않는 프로젝트는 PROJECT_NOT_FOUND를 반환한다", async () => {
  ...
  it("(c) agent=CODEX 페이로드 → 세션 create 에 agent=CODEX 기록", async () => {
    ...
    expect(db.claudeSession.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        create: expect.objectContaining({ agent: "CODEX" }),
      }),
    );
  });
  ...
  it("(d) agent 미지정 → 세션 create 에 agent=CLAUDE (후방호환)", async () => {
    ...
    expect(db.claudeSession.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        create: expect.objectContaining({ agent: "CLAUDE" }),
      }),
    );
  });
});
