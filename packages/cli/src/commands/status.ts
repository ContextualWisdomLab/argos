 ...
 */
export const makeStatusCommand: CommandFactory = (deps) => async () => {
  ...
  if (project) {
    ...
    console.log(
      "  API URL:  " + (project.apiUrl ?? config?.apiUrl ?? DEFAULT_API_URL),
    );
  } else {
  ...
  const claudePath = // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    join(deps.cwd(), ".claude", "settings.json");
  const codexPath = // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    join(deps.cwd(), ".codex", "hooks.json");
