...
describe("ERDModel", () => {
  ...
  describe("Table Management", () => {
    ...
    it("should return a deep copy to prevent mutation bypass", () => {
      const table = model.addTable("users");
      table.name = "hacked";
      expect(model.getTable("users")?.name).toBe("users");
    });
  ...
  describe("Column Management", () => {
    ...
    it("should reject unsafe SQL types", () => {
      model.addTable("users");
      expect(() =>
      ...
      ).toThrowError("Unsafe SQL type: 'INT; DROP TABLE users;'");
    });

    it("should reject unsafe default values", () => {
      model.addTable("users");
      expect(() =>
        model.addColumn("users", {
          name: "id",
      ...
      ).toThrowError("Unsafe default value: '1; DROP TABLE users;'");
    });
  });
