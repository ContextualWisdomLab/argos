...
## 2024-11-21 - 장식용 아이콘 스크린 리더 접근성 개선
**Learning:** `lucide-react`와 같은 SVG 기반 아이콘을 정보 전달이 아닌 순수 장식용(예: 버튼 옆, 데이터 카드 제목 앞 등)으로 사용할 때, `aria-hidden="true"`를 명시적으로 추가하지 않으면 스크린 리더가 불필요하게 아이콘의 존재를 인지하거나 이름 없는 요소로 읽으려 시도할 수 있습니다. 이는 사용자 경험을 저해하는 요인이 됩니다.
**Action:** 순수 장식용 아이콘을 사용할 때는 반드시 `<Icon className="..." aria-hidden="true" />` 패턴을 적용하여 스크린 리더에서 무시되도록 처리해야 합니다. 또한, 테스트 단계에서 `toHaveAccessibleName` 등을 활용해 버튼이나 컴포넌트가 의도한 접근성 이름을 올바르게 가지고 있는지 검증하는 습관을 들여야 합니다.
