  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // ⚡ Bolt Optimization: Use for...in instead of Object.keys() in extremely hot paths.
        // Impact: Avoids array allocation for the keys entirely, significantly reducing GC overhead when aggregating large daily rollups.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // ⚡ Bolt Optimization: Replace Object.keys() with for...in for extreme hot paths
    // Object.keys() still creates an array of keys. Using for...in avoids all array allocations entirely,
    // dramatically reducing garbage collection overhead when aggregating thousands of rollups.
    for (const k in r.skillCounts) {
      if (Object.hasOwn(r.skillCounts, k)) {
