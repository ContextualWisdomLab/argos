...
- 타임라인 최적화 PR을 현행 dependency owner에 재스택해 `deepmerge-ts` 7.1.5의 High 취약점을 수정된 8.0.2 계열로 교체하고 중복 OSV workflow를 중앙 Security Scan 경계로 통합했습니다.
...
- 세션 타임라인 정렬 전에 각 사용량 timestamp를 정확히 한 번 파싱하도록 고정하고, 네 항목이 네 번만 파싱되는 회귀 테스트로 comparator 내부 재파싱을 차단했습니다.
