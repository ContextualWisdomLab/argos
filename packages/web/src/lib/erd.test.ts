...
describe("ERDModel", () => {
  ...
  describe("Column Management", () => {
    ...
    it("should throw when column type contains a semicolon (SQL Injection prevention)", () => {
      model.addTable("users");
      expect(() =>
        model.addColumn("users", {
          name: "id",
          type: "integer; DROP TABLE users;",
        }),
      ).toThrowError(
        "Invalid column type 'integer; DROP TABLE users;': Statement terminators like ';' are not allowed to prevent SQL injection.",
      );
    });
  });
