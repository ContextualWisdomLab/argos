...
export async function getWeeklyReport(
  ...
  // Insights — delegation
  // ⚡ Bolt Optimization:
  // 병목 지점: 기존 코드는 `thisWeekRollups`를 여러 번 순회하고, 매 순회마다 Object.keys()로 중간 배열을 생성하여 메모리 할당 비용이 발생했습니다.
  // 최적화 방법: 단일 for...of 루프와 for...in 루프를 결합하여 중간 배열 할당을 제거했습니다.
  // 기대 효과: `thisWeekRollups`의 크기가 클 경우, 불필요한 배열 생성 오버헤드와 가비지 컬렉션을 줄여 리포트 생성 성능이 향상됩니다.
  ...
  for (const r of thisWeekRollups) {
    for (const k in r.agentCounts) {
      if (Object.prototype.hasOwnProperty.call(r.agentCounts, k)) {
        totalAgentCalls += r.agentCounts[k]!
      }
    }
    for (const k in r.skillCounts) {
      if (Object.prototype.hasOwnProperty.call(r.skillCounts, k)) {
