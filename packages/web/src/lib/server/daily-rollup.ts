  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // [Bolt: Performance Optimization] Use guarded for...in instead of Object.keys() in hot paths.
        // Impact: Avoids array allocation entirely for object keys, significantly reducing GC overhead when aggregating large daily rollups.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] Guarded for...in avoids all array allocations, reducing heap thrashing entirely
