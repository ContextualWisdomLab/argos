...
export function findProjectConfigWithPath(
  ...
  let currentDir = resolve(startDir || process.cwd()) // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- this API intentionally starts from a caller-selected local directory.
  let depth = 0
  const maxDepth = 10

  while (depth < maxDepth) {
    const configPath = join(currentDir, '.argos', 'project.json') // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- currentDir is the bounded upward traversal root.
...
export function writeProjectConfig(config: ProjectConfig, dir?: string): void {
  ...
  const argosDir = join(targetDir, '.argos') // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- targetDir is the caller-selected project root.

  if (!existsSync(argosDir)) {
    mkdirSync(argosDir, { recursive: true })
  }

  const configPath = join(argosDir, 'project.json') // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- fixed child of the selected project root.
  writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf8')

  // Create .gitignore with comment (but don't actually ignore anything)
  const gitignorePath = join(argosDir, '.gitignore') // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- fixed child of the selected project root.
