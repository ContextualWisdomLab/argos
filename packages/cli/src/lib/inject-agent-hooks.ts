...
export function injectAgentHooks(deps: ExternalDeps, cwd: string): AgentHookResult {
  return {
    claude: deps.hooks.inject(join(cwd, '.claude', 'settings.json'), 'claude'), // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- caller intentionally selects the local project root.
    codex: deps.hooks.inject(join(cwd, '.codex', 'hooks.json'), 'codex'), // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- caller intentionally selects the local project root.
  }
}
