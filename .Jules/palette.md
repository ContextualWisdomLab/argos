...
## 2024-08-16 - 비동기 액션 버튼의 스크린 리더 피드백 누락 패턴
**Learning:** 비동기 작업(생성, 로딩, 복사 등) 처리 시 버튼의 `disabled` 상태나 텍스트 변경(예: 'Creating...')은 시각적으로는 잘 표현되지만, `aria-live` 및 `aria-busy` 속성이 누락되어 스크린 리더 사용자는 상태 변화를 즉시 인지하지 못하는 패턴을 확인했습니다.
**Action:** 상태가 변하는 비동기 처리 버튼과 로딩 인디케이터에는 항상 `aria-busy={isLoading}`와 `<span aria-live="polite">`를 함께 적용하여, 시각적 피드백과 동등한 접근성 피드백을 제공하도록 합니다.
