...
**Vulnerability:** The application exports data to CSV without sanitizing fields that start with formula indicators (`=`, `+`, `-`, `@`, `\t`, `\r`). This allows malicious users to inject spreadsheet formulas which could execute arbitrary commands or leak data when opened in spreadsheet software like Microsoft Excel or Google Sheets.
**Learning:** Raw string conversion and CSV encoding rules (wrapping in double quotes and escaping inner quotes) do not protect against Spreadsheet Macro/Formula Injection if the field content starts with a formula trigger character. Additionally, formatting protection should not be applied to raw numbers.
**Prevention:** Always prepend a single quote (`'`) to any string field values starting with `=`, `+`, `-`, `@`, `\t`, or `\r` before writing them to CSV exports to ensure the spreadsheet application treats the content strictly as text. Ensure that values of type `number` are skipped to preserve numeric formatting and functionality (like `SUM()`).
