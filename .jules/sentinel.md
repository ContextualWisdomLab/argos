...
**Vulnerability:** The application was directly formatting user-provided inputs (like session titles or prompts) into a CSV export without escaping formula characters. This allows a malicious user to supply input starting with `=`, `+`, `-`, `@`, `\t`, or `\r`, which executes as a macro/formula when the downloaded CSV is opened in spreadsheet programs (CSV Injection/Spreadsheet Macro Injection).
**Learning:** Even though CSV is a plain text format, modern spreadsheet programs treat certain initial characters as formulas, leading to unexpected and potentially dangerous command execution on the victim's local machine.
**Prevention:** Always prepend a single quote (`'`) to any CSV field values that begin with potentially dangerous characters (`/^[=+\-@\t\r]/`) to force the spreadsheet application to treat the content as text instead of a formula.
