...
- ERD DDL 생성 경계에서 임의 SQL 토큰 스트림을 타입으로 허용하지 않고, 단일 타입 식별자와 명시적으로 허용한 다중 단어 타입 문법만 수용하도록 강화했습니다. 기본값은 스칼라 리터럴과 제한된 안전 내장 표현식만 허용하며, 검증된 컬럼·외래키 입력을 저장 전에 복사해 호출자 보유 참조를 통한 검증 우회 상태 변이를 차단했습니다. SQL 구조 allow-list와 상태 격리 회귀 테스트 및 OWASP/PostgreSQL 근거 문서를 함께 추가했습니다.
