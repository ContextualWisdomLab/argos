...
- 세션 타임라인 사용량 시점의 `Date.parse`를 정렬 비교기 밖으로 이동해 각 timestamp를 정확히 한 번만 파싱합니다. 시간순 결과, 누적 도구 요약 및 입력 불변성은 유지하면서 비교기 호출 수에 비례하던 반복 문자열 파싱을 제거했고, 호출 횟수 회귀 테스트로 선형 파싱 예산을 고정했습니다.
