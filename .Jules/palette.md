...
## 2024-03-20 - [ARIA-Live Region Reliability for Button States]
**Learning:** [Mutating the text inside an active focus target (like a button) with `aria-live="polite"` can be unreliable in screen readers, and often causes double-reading when `aria-pressed` is used.]
**Action:** [Use a stable, generic `aria-label` for the button's accessible name, and use a separate visually hidden sibling `<span role="status" aria-atomic="true" className="sr-only">` to confidently announce temporary state changes without interrupting button focus context.]
