...
## 2025-02-18 - 비동기 버튼 로딩 상태의 접근성 및 시각적 피드백 개선
**Learning:** `CreateProjectModal`과 같이 비동기 작업을 수행하는 모달에서, 작업이 진행 중일 때 단순히 "생성 중…"과 같은 텍스트만 표시하면 사용자가 작업이 정상적으로 처리되고 있는지 명확히 인지하기 어렵습니다. 또한 스크린 리더 사용자를 위해 로딩 스피너와 같은 시각적 아이콘은 무시되도록 처리해야 합니다.
**Action:** 비동기 작업을 트리거하는 버튼에는 `Loader2`와 같은 스피너 아이콘에 `animate-spin` 클래스를 적용하여 명확한 시각적 피드백을 제공하고, 아이콘에는 반드시 `aria-hidden="true"`를 추가하여 스크린 리더가 불필요하게 스피너를 읽지 않도록(텍스트만 읽도록) 개선하는 패턴을 지속적으로 적용해야 합니다.
