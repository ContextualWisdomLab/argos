...
## 2024-06-25 - CopyPromptButton 접근성 개선
**Learning:** 버튼의 텍스트가 동적으로 변경될 때, 포커스된 엘리먼트 내부의 텍스트를 직접 변경하면 스크린 리더가 상태 변화를 제대로 인지하기 어렵습니다.
**Action:** 버튼 내부 텍스트를 aria-hidden으로 가려 시각적으로만 보여주고, sr-only 클래스를 사용하여 접근 가능한 이름을 고정합니다. 그 외에 role="status" 및 aria-atomic="true"를 가진 별도의 형제(sibling) 요소를 사용하여 스크린 리더에게 상태 업데이트를 명확하게 전달합니다.
