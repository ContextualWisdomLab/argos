...
- CLI 브라우저 인증 시작점에서 운영체제 실행기로 전달되는 URL을 WHATWG URL로 정규화하고, 자격 증명·제어문자·HTTP(S) 이외의 프로토콜을 거부합니다. Windows에서는 `cmd.exe /c start`와 `windowsVerbatimArguments`를 제거하고 검증된 URL 하나만 `explorer.exe`에 직접 전달하여 셸 명령 경계를 없앴으며, 플랫폼별 실행 계약과 악성 프로토콜 회귀 테스트를 추가했습니다.
