...
The product treats these values as local calendar dates rather than elapsed UTC instants. Query values must match `yyyy-MM-dd`, parse to the same real calendar date, and form a non-reversed interval. `resolveSessionDateRange` is the shared contract for the picker, the sessions data query, and CSV export. If either bound is absent, malformed, impossible, or ordered after the other bound, all three surfaces fail closed to the same canonical seven-day interval. Preset selection and pressed-state calculation use calendar-day differences, so daylight-saving transitions do not change the advertised inclusive day count.
...
Rollback the date-range descriptions and shared URL validation, session-target labels, focused tests, this doctoring note, and the matching Unreleased changelog entry together. Do not roll back the pre-existing `aria-pressed` state or keyboard focus ring.
