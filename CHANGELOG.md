...
- ERD DDL 생성 경계를 구조적 allow-list 방식으로 강화했습니다. 지원하는 PostgreSQL 타입 문법과 제한된 side-effect-free 기본값만 허용하고, 컬럼 제약·임의 함수 호출·잘못된 런타임 타입을 거부합니다. 검증된 컬럼·외래 키는 모델 소유 복사본으로 저장하며, 모든 식별자를 quoted identifier로 출력해 예약어 충돌과 검증 후 객체 변이 우회를 차단합니다. 관련 회귀 테스트와 `docs/doctoring/erd-ddl-injection-boundary.md`에 위협 모델·롤백·APA 7 근거를 기록했습니다.
