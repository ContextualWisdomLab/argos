...
**Learning:** Even when inputs are properly enclosed in double quotes (to escape commas), spreadsheet software will still evaluate formulas if the content begins with `=` , `+`, `-`, or `@`. Furthermore, leading whitespace before these characters is ignored by parsers, allowing bypasses like `  =1+1`.
**Prevention:** Always sanitize fields intended for CSV export by prepending a single quote (`'`) to values that start with `=` , `+`, `-`, or `@` (accounting for leading spaces/tabs via regex `/^[\s]*[=+\-@\t\r]/`).
