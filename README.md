...
cd your-project
argos    # 브라우저 로그인 → 조직·프로젝트 초기화까지 한 번에 (Claude Code · Codex hook 동시 설치)
...
> **Codex 사용자**: Codex 는 보안상 새 hook 을 자동 실행하지 않습니다. `argos` 설치 후 codex 를 한 번 실행하고
...
Codex 를 쓴다면, codex 실행 후 `/hooks` 에서 argos hook 들을 trust 하라고 사용자에게 안내할 것.
기타 파일은 수정하지 마.
```
...
1. `argos`를 처음 실행하면 브라우저 OAuth로 로그인하고, 조직·프로젝트를 초기화합니다 (`.argos/project.json` 생성 + `.claude/settings.json`·`.codex/hooks.json`에 훅 항목 추가).
2. 이후 Claude Code / Codex 가 훅을 발사할 때마다 `argos hook`이 호출되어 세션 메타·토큰·툴 호출·전사를 Argos API로 전송합니다. (Codex 는 rollout 포맷이 달라 `argos hook --agent codex` 가 전용 파서로 분기.)
3. 팀원은 저장소를 pull한 뒤 `argos`를 한 번 실행하면 같은 조직으로 자동 합류합니다. (Codex 는 `/hooks` trust 1회 필요.)
