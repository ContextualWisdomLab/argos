...
export async function getDailyRollupsForProjects(
  ...
  const userStatsMapsByDate = new Map<string, Map<string, DailyUserStat>>()
  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        const map = new Map<string, DailyUserStat>()
        for (const u of r.userStats) map.set(u.userId, { ...u })
        userStatsMapsByDate.set(r.date, map)
      } else {
        ...
        const userMap = userStatsMapsByDate.get(r.date)!
