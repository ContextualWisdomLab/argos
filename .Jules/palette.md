...
## 2024-05-24 - Screen reader redundance on standard chevron icons
**Learning:** Found multiple instances where purely decorative `Chevron` icons (lucide-react) inside interactive elements (buttons, pagination, popovers) lacked `aria-hidden="true"`. This causes screen readers to unnecessarily announce the icon shape when navigating.
**Action:** Always append `aria-hidden="true"` to SVG/icon components when they serve only as visual embellishments to a functional text label or are already wrapped in an interactive element with a descriptive `aria-label`.
