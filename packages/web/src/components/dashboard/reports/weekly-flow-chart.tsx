...
function CustomTooltip({
  ...
  const thisWeek =
    payload.find((p) => p.dataKey === "thisWeekTokens")?.value ?? 0;
  const prevWeek =
    payload.find((p) => p.dataKey === "prevWeekTokens")?.value ?? 0;
  return (
    <div className="rounded-lg border border-border bg-popover text-popover-foreground shadow-lg p-3">
      ...
      <div className="space-y-1 text-sm">
        <Row
          ...
          label="이번 주"
          value={Number(thisWeek)}
        />
        <Row
          color="var(--color-chart-2)"
          label="지난 주"
          value={Number(prevWeek)}
        />
      </div>
    </div>
  );
...
export function WeeklyFlowChart({ thisWeek, prevWeek }: WeeklyFlowChartProps) {
  const data = useMemo(() => {
    ...
    const result: ChartRow[] = dayNames.map((day) => ({
      day,
      thisWeekTokens: 0,
      prevWeekTokens: 0,
    }));
