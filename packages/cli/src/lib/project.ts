...
export function findProjectConfigWithPath(
  ...
  // nosemgreplet currentDir = resolve(startDir || process.cwd())
  let currentDir = resolve(startDir || process.cwd())
  let depth = 0
  const maxDepth = 10

  while (depth < maxDepth) {
    // nosemgrepconst configPath = join(currentDir, '.argos', 'project.json')
...
export function writeProjectConfig(config: ProjectConfig, dir?: string): void {
  ...
  // nosemgrepconst argosDir = join(targetDir, '.argos')
