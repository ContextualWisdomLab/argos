...
- Admin impersonation은 같은 출처의 `/admin/impersonate?token=...`만 허용합니다. 교차 출처, 자격 증명, 추가 쿼리, hash는 이동하지 않습니다. 고객을 바꾸면 진행 중인 재설정 링크·가장 세션 요청은 UI에 적용되지 않습니다. 잘못된 고객의 링크를 복사하거나 잘못된 세션으로 들어가기 전에, 지금 선택된 고객에서 동작을 다시 시작하십시오.
