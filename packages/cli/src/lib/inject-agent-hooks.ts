...
export function injectAgentHooks(deps: ExternalDeps, cwd: string): AgentHookResult {
  return {
    claude: deps.hooks.inject(/* nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal */ join(cwd, '.claude', 'settings.json'), 'claude'),
    codex: deps.hooks.inject(/* nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal */ join(cwd, '.codex', 'hooks.json'), 'codex'),
  }
}
