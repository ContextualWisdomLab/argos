...
## 2024-05-10 - Resolve CI Failures: Vulnerable Dependencies & SAST path-traversal
**Vulnerability:** The Dependency Review/Trivy tools flagged vulnerable packages (next.js, @auth/core, hono, etc) leading to a build failure. Additionally, Semgrep reported multiple `javascript.lang.security.audit.path-traversal` and `python.lang.security.audit.dynamic-urllib-use-detected` warnings inside `packages/cli` and `.claude/skills` scripts.
**Learning:** Upgrading vulnerable subdependencies through the lockfile (`pnpm up -r <packages>`) is required to clear security scanner checks. For SAST warnings like `path.join` traversal, if the base path is intrinsically safe (e.g., `process.cwd()` or a controlled `tempDir`), you can suppress the false positive via `// nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal`.
**Prevention:** Regularly run `pnpm up` or use dependabot to keep workspace dependencies updated against known CVEs. When writing scripts using `path.join` or `urllib.request.urlopen`, either explicitly validate the dynamic inputs (e.g., URL scheme allowlisting, checking if paths are relative) or use explicit inline suppressions (`// nosemgrep` or `# nosemgrep`) to signify to the security suite that the use case is intentional and controlled.
