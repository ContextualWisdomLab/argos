...
## 2024-05-10 - Asynchronous Copy Feedback and Async Test Rendering
**Learning:** In Next.js/React applications, when adding accessible and visually responsive micro-UX like swapping a 'Copy' icon for a 'Check' icon after a clipboard action, it is important to wrap `navigator.clipboard.writeText` in a try-catch block to prevent unhandled promise rejections if the Clipboard API is unavailable. When testing components that rely on debounced inputs and asynchronous fetch requests, simply wrapping trigger events in `await React.act()` is not enough. You must mock the global `fetch` API, use `vi.useFakeTimers()`, advance the timers explicitly past the debounce window (`vi.advanceTimersByTime(200)`), and then `await waitFor` for the state to settle and the DOM to re-render.
**Action:** When creating tests for components with debounced search or async fetches, proactively set up fake timers, mock the global fetch API, and carefully sequence `act()`, timer advancement, and DOM assertion to avoid timeout failures and achieve 100% coverage reliably.
