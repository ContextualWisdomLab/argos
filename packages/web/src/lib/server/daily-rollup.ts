  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // [Bolt: Performance Optimization] Use for...in loops instead of Object.keys() or Object.entries() in hot paths.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] for...in loops avoid array allocations, reducing heap thrashing
    for (const k in r.skillCounts) {
