...
CREATE TABLE posts (
  ...
  describe("Renaming", () => {
    ...
    it("should rename a column and update foreign key references", () => {
    ...
    it("should rename a foreign key column and update internal references", () => {
      model.renameColumn("posts", "user_id", "author_id");

      const postsTable = model.getTable("posts");
      expect(postsTable?.columns[0].name).toBe("author_id");
      expect(postsTable?.foreignKeys[0].columnName).toBe("author_id");
    });

    it("should throw when renaming a non-existent column", () => {
      expect(() => model.renameColumn("users", "non_existent", "new_col")).toThrowError(
        "Column 'non_existent' does not exist in table 'users'.",
      );
    });

    it("should throw when renaming a column to an existing name", () => {
