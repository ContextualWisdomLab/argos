...
- 조직이 없는 상태의 로그아웃 제어를 명시적인 비제출 버튼으로 고정하고 키보드 `focus-visible` 표시를 추가했으며, 텍스트가 이미 이름을 제공하는 조직 생성 버튼의 `PlusIcon`을 접근성 트리에서 숨겼습니다. 실제 클릭·모달 전환을 포함한 DOM 회귀 테스트로 계약을 고정했습니다.
