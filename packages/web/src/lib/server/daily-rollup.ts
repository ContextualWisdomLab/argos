  ...
  for (const rollups of perProject) {
    for (const r of rollups) {
      ...
      if (!prev) {
        ...
        // Impact: Avoids array allocation for keys, significantly reducing GC overhead when aggregating large daily rollups.
...
export function aggregateSummary(
  ...
  for (const r of rollups) {
    ...
    // [Bolt: Performance Optimization] for...in iterations avoid Object.keys() array allocations, reducing GC overhead in hot paths
    for (const k in r.skillCounts) {
