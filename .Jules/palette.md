...
## 2024-11-20 - CopyPromptButton 접근성 향상 (스크린 리더 상태 발표 최적화)
**Learning:** 포커스된 버튼의 텍스트가 시각적으로 변경될 때, `aria-live` 속성을 직접 사용하여 접근성 이름을 변경하면 스크린 리더에서 이중으로 읽어주거나 일관성 없는 동작이 발생할 수 있습니다.
**Action:** 시각적 텍스트와 스크린 리더용 텍스트를 분리해야 합니다. 시각적 텍스트는 `aria-hidden="true"`로 숨기고, 스크린 리더용 안내 텍스트는 별도의 형제 요소로 `role="status"` 와 `aria-atomic="true"`를 가지며 `sr-only` (시각적 숨김) 클래스를 적용하여 추가해야 합니다.
