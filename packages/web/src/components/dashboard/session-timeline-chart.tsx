...
function buildChartData(
  ...
  // [Bolt: Performance Optimization] Use Schwartzian transform (map-sort-map) to avoid calling Date.parse O(N log N) times inside the sort comparator.
  const mappedUsage = usageTimeline.map(usage => ({
    original: usage,
    parsedTimestamp: Date.parse(usage.timestamp)
  }))
  mappedUsage.sort((a, b) => a.parsedTimestamp - b.parsedTimestamp)
  ...
  return mappedUsage.map(({ original: usage, parsedTimestamp }) => {
    while (
