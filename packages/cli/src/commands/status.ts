...
export const makeStatusCommand: CommandFactory = (deps) => async () => {
  ...
  const cwdUrl = pathToFileURL(`${deps.cwd()}/`);
  const claudePath = fileURLToPath(new URL(".claude/settings.json", cwdUrl));
  const codexPath = fileURLToPath(new URL(".codex/hooks.json", cwdUrl));
