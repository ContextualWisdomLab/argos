...
- 세션 CSV 내보내기에서 사용자·프로젝트명, 세션 제목과 최초 프롬프트처럼 신뢰할 수 없는 문자열이 `=`, `+`, `-`, `@`, 탭, CR 또는 LF로 시작하면 스프레드시트 수식으로 실행되지 않도록 탭 접두어를 붙이고 RFC 4180 방식으로 인용합니다. 숫자형 집계값은 숫자로 유지하며, 쉼표·큰따옴표·줄바꿈 및 모든 수식 접두어를 직접 검증하는 회귀 테스트와 운영 근거 문서를 추가했습니다.
