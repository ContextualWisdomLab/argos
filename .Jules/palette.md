...
## 2025-03-05 - 동적 토글 버튼의 명확한 상태 전달 패턴
**Learning:** 버튼 텍스트나 아이콘 변경만으로는 화면 낭독기(스크린 리더) 사용자가 토글될 콘텐츠의 목적과 열림/닫힘 액션을 명확하게 파악하기 어려울 수 있습니다. 특히 "What do these numbers mean?" 처럼 질문 형태의 버튼이나, 아이콘으로만 구성된 요약 버튼의 경우 더욱 그렇습니다.
**Action:** 토글 버튼이나 요약 정보 뱃지 버튼에 상태(`open`, `expanded` 등)나 컨텍스트를 반영한 동적인 `aria-label`을 제공하여 (예: `{expanded ? "설명 닫기" : "설명 열기"}`) 시각 장애인 사용자의 상호작용 명확성을 높입니다.
