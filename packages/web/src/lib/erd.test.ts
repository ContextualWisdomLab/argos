...
describe("ERDModel", () => {
  ...
  describe("Table Management", () => {
    it("should add a new table", () => {
      ...
      expect(model.getTable("users")).toEqual(table);
    });

    it("does not expose mutable table state", () => {
      const table = model.addTable("users");
      table.name = "renamed_users";
      table.columns.push({ name: "injected_column", type: "TEXT" });

      const listed = model.getTables();
      listed[0].columns.push({ name: "another_injected_column", type: "TEXT" });

      expect(model.getTable("users")).toEqual({
        name: "users",
        columns: [],
        foreignKeys: [],
      });
  ...
  describe("Column Management", () => {
    it("should add a column to an existing table", () => {
      ...
      expect(table?.columns[0]).toEqual({ name: "id", type: "integer" });
    });
