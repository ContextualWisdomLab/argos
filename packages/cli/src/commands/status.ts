...
export const makeStatusCommand: CommandFactory =
  (deps) => async () => {
    ...
    // Advisory (.audit.) false positive: paths are the CLI's own working
    // directory (deps.cwd()) joined with fixed literals — not attacker input.
    const claudePath = join(deps.cwd(), '.claude', 'settings.json') // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    const codexPath = join(deps.cwd(), '.codex', 'hooks.json') // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
