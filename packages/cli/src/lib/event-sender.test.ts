...
describe("buildSelfHealScript", () => {
  ...
  it("holds an inter-process lock while rewriting project.json", () => {
    expect(script).toContain(
      `const lockDir=${JSON.stringify(PROJECT_JSON_PATH)}+'.lock'`,
    );
    const mkdirIdx = script.indexOf("fs.mkdirSync(lockDir)");
    const readIdx = script.indexOf(
      `JSON.parse(fs.readFileSync(${JSON.stringify(PROJECT_JSON_PATH)},'utf8'))`,
    );
    const renameIdx = script.indexOf(
      `fs.renameSync(atomicTmp,${JSON.stringify(PROJECT_JSON_PATH)})`,
    );
