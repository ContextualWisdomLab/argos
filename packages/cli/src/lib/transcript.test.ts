...
describe('readTranscriptLines', () => {
  ...
  beforeEach(() => {
    // nosemgreptempDir = mkdtempSync(join(tmpdir(), 'argos-rtl-'))
  ...
  it('파일이 없으면 빈 배열을 반환한다', async () => {
    // nosemgrepconst result = await readTranscriptLines(join(tempDir, 'nonexistent.jsonl'))
  ...
  it('파싱 실패한 줄은 {} 로 반환한다', async () => {
    // nosemgrepconst path = join(tempDir, 'bad.jsonl')
  ...
  it('빈 줄은 제거한다', async () => {
    // nosemgrepconst path = join(tempDir, 'empty-lines.jsonl')
...
describe('extractUsageFromTranscript', () => {
  ...
  beforeEach(() => {
    // nosemgreptempDir = mkdtempSync(join(tmpdir(), 'argos-usage-'))
...
describe('detectSlashCommand', () => {
  ...
  beforeEach(() => {
    // nosemgreptempDir = mkdtempSync(join(tmpdir(), 'argos-slash-'))
...
describe('extractMessages', () => {
  ...
  beforeEach(() => {
    // nosemgreptempDir = mkdtempSync(join(tmpdir(), 'argos-msg-'))
