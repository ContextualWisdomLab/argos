...
## 2025-01-22 - 원시 `<button>` 대신 디자인 시스템 `<Button>` 사용으로 접근성/일관성 개선
**Learning:** `cli-auth/client.tsx`의 "허용", "거부" 버튼과 같이 원시 `<button>` 태그에 직접 Tailwind 클래스를 주입하여 만든 버튼은, 프로젝트 전반에 설정된 키보드 포커스 링(`focus-visible`)이나 비활성화(`disabled`) 상태의 일관된 디자인 시스템 룰을 누락하기 쉽습니다. 특히 키보드 사용자를 위한 접근성 처리가 빠질 가능성이 큽니다.
**Action:** 사용자 인터랙션이 발생하는 주요 액션 버튼을 구현할 때는 원시 HTML `<button>` 태그를 사용하지 말고, 항상 `@/components/ui/button`과 같이 이미 접근성(`aria` 속성, `focus-visible` 등)과 디자인 일관성이 확보된 공용 `<Button>` 컴포넌트를 사용해야 합니다.
