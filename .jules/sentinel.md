...
## 2025-02-18 - [Fix CSV Formula Injection in Data Export]
**Vulnerability:** The CSV export functionality for dashboard sessions (`packages/web/src/app/api/orgs/[orgSlug]/dashboard/sessions/route.ts`) directly serialized user-controlled inputs (like names and prompts) without sanitizing prefix characters. This exposes users to CSV Formula Injection (also known as Spreadsheet Macro Injection), where a malicious user inputs `=cmd|' /C calc'!A0` and compromises a victim opening the CSV.
**Learning:** Raw string conversion and CSV quote-escaping (`"`) are not enough to protect against Spreadsheet injection. Excel and other spreadsheet software evaluate cells starting with `=`, `+`, `-`, or `@` as executable formulas regardless of CSV quoting.
**Prevention:** Always sanitize data being exported to CSV by checking if it starts with characters like `=`, `+`, `-`, `@`, `\t`, or `\r`. Prepend a single quote (`'`) to force the spreadsheet application to treat the content as literal text.
