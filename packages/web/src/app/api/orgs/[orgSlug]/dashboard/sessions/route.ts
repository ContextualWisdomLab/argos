...
type SessionWithInclude = Prisma.ClaudeSessionGetPayload<{
...
function mapSessionItem(session: SessionWithInclude): SessionItem {
  ...
  const fallbackTitle =
    session.messages[0]?.content.slice(0, 200).trim() || null;
...
function buildSessionsCsv(sessions: SessionWithInclude[]) {
  ...
  const rows = sessions.map((session) => {
    ...
    const title =
      session.title?.trim() ||
      session.messages[0]?.content.slice(0, 200).trim() ||
      "";
...
export async function GET(
  ...
  try {
    ...
    const projectIds = await resolveOrgScopedProjectIds(
      access.org.id,
      userId,
      access.role,
      projectIdParam,
    ...
    if (projectIds.length === 0) {
      ...
      const body: PaginatedResult<SessionItem> = {
        items: [],
        total: 0,
        page,
        pageSize,
    ...
    if (wantsCsv) {
      ...
      return new NextResponse(buildSessionsCsv(csvSessions), {
        headers: csvHeaders,
      });
    }
